...
int lkmd_strnicmp(const char *s1, const char *s2, size_t len)
{
...
struct lkmd_kernel_sym {
	unsigned long curr_task;
	unsigned long irq_enter;
	unsigned long irq_exit;
	unsigned long kallsyms_lookup;
	unsigned long find_extend_vma;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,9,0)
	unsigned long follow_page_mask;
#endif
};

static struct lkmd_kernel_sym kernelsym;

int __init lkmd_kernsym_init(void)
{
	if ((kernelsym.curr_task = kallsyms_lookup_name("curr_task")) == 0 ||
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,9,0)
			(kernelsym.follow_page_mask = kallsyms_lookup_name("follow_page_mask")) == 0 ||
#endif
			(kernelsym.irq_enter = kallsyms_lookup_name("irq_enter")) == 0 ||
			(kernelsym.irq_exit = kallsyms_lookup_name("irq_exit")) == 0 ||
			(kernelsym.kallsyms_lookup = kallsyms_lookup_name("kallsyms_lookup")) == 0 ||
			(kernelsym.find_extend_vma = kallsyms_lookup_name("find_extend_vma")) == 0)
		return -EFAULT;

	if ((orig_smp_error_interrupt = (void *)kallsyms_lookup_name("smp_error_interrupt")) == 0 ||
			(orig_do_debug = (void *)kallsyms_lookup_name("do_debug")) == 0 ||
			(orig_do_int3 = (void *)kallsyms_lookup_name("do_int3")) == 0)
		return -EFAULT;

    return 0;
}

struct task_struct *lkmd_curr_task(int p)
{
	struct task_struct *(*fn)(int) = (void *)kernelsym.curr_task;
	return fn(p);
}

void lkmd_irq_enter(void)
{
	void (*fn)(void) = (void *)kernelsym.irq_enter;
	fn();
}

void lkmd_irq_exit(void)
{
	void (*fn)(void) = (void *)kernelsym.irq_exit;
	fn();
}

const char *lkmd_kallsyms_lookup(unsigned long addr,
			    unsigned long *symbolsize,
			    unsigned long *offset,
			    char **modname, char *namebuf)
{
	const char *(*fn)(unsigned long,
		unsigned long *, unsigned long *, 
        char **, char *) = (void *)kernelsym.kallsyms_lookup;
	return fn(addr, symbolsize, offset, modname, namebuf);
}

struct vm_area_struct *lkmd_find_extend_vma(struct mm_struct *mm, unsigned long addr)
{
	struct vm_area_struct *(*fn)(struct mm_struct *, 
        unsigned long) = (void *)kernelsym.find_extend_vma;
	return fn(mm, addr);
}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,9,0)
struct page *lkmd_follow_page(struct vm_area_struct *vma,
                              unsigned long address, unsigned int flags)
{
	unsigned int unused_page_mask;
	struct page *(*fn)(struct vm_area_struct *,unsigned long, unsigned int,
    	unsigned int *) = (void *)kernelsym.follow_page_mask;

	return fn(vma, address, flags, &unused_page_mask);
}
#else
#define lkmd_follow_page(vma, address, flags) follow_page(vma, address, flags)
#endif

int kdb_task_has_cpu(const struct task_struct *p)
{
	return (lkmd_curr_task(task_cpu(p)) == p);
}

struct task_struct *lkmd_find_task_by_pid_ns(pid_t nr, struct pid_namespace *ns)
{
...
// const char *lkmd_walk_kallsyms(loff_t *pos)
// {
...
int lkmd_get_sym_val(const char *symname, kdb_symtab_t *symtab)
{
	if (KDB_DEBUG(AR))
		lkmd_printf("lkmd_get_sym_val: symname=%s, symtab=%p\n", symname, symtab);
	memset(symtab, 0, sizeof(*symtab));

	if ((symtab->sym_start = kallsyms_lookup_name(symname))) {
		if (KDB_DEBUG(AR))
			lkmd_printf("lkmd_get_sym_val: returns 1, symtab->sym_start=0x%lx\n", symtab->sym_start);
		return 1;
	}
	if (KDB_DEBUG(AR))
		lkmd_printf("lkmd_get_sym_val: returns 0\n");
	return 0;
}
...
#define knt1_size 128		/* must be >= kallsyms table size */
	...
	if (KDB_DEBUG(AR))
		lkmd_printf("kdbnearsym: addr=0x%lx, symtab=%p\n", addr, symtab);
	memset(symtab, 0, sizeof(*symtab));
	...
	if (!knt1) {
		lkmd_printf("kdbnearsym: addr=0x%lx cannot kmalloc knt1\n", addr);
		goto out;
	}
	symtab->sym_name = lkmd_kallsyms_lookup(addr, &symbolsize , &offset, (char **)(&symtab->mod_name), knt1);
	...
	if (KDB_DEBUG(AR))
		lkmd_printf("kdbnearsym: returns %d symtab->sym_start=0x%lx, symtab->mod_name=%p, symtab->sym_name=%p (%s)\n", ret, symtab->sym_start, symtab->mod_name, symtab->sym_name, symtab->sym_name);
...
// 	while ((name = lkmd_walk_kallsyms(&pos))) {
...
int kdb_ipi(struct pt_regs *regs, void (*ack_interrupt)(void))
{
...
void kdb_symbol_print(kdb_machreg_t addr, const kdb_symtab_t *symtab_p, unsigned int punc)
{
	...
	if (symtab_p2->sym_name) {
		...
		if (strcmp(symtab_p2->mod_name, "kernel")) {
			lkmd_printf("[%s]", symtab_p2->mod_name);
		}
		lkmd_printf("%s", symtab_p2->sym_name);
		if (addr != symtab_p2->sym_start) {
			lkmd_printf("+0x%lx", addr - symtab_p2->sym_start);
		}
		if (punc & KDB_SP_SYMSIZE) {
			lkmd_printf("/0x%lx", symtab_p2->sym_end - symtab_p2->sym_start);
		}
		if (punc & KDB_SP_PAREN) {
			lkmd_printf(")");
		}
...
int kdb_getarea_size(void *res, unsigned long addr, size_t size)
{
	...
	if (ret) {
		if (!KDB_STATE(SUPPRESS)) {
			lkmd_printf("kdb_getarea: Bad address 0x%lx\n", addr);
...
int kdb_putarea_size(unsigned long addr, void *res, size_t size)
{
	...
	if (ret) {
		if (!KDB_STATE(SUPPRESS)) {
			lkmd_printf("kdb_putarea: Bad address 0x%lx\n", addr);
			KDB_STATE_SET(SUPPRESS);
		}
		ret = KDB_BADADDR;
	}
...
int kdb_getphysword(unsigned long *word, unsigned long addr, size_t size)
{
	...
	*word = 0;	/* Default value if addr or size is invalid */
	...
	default:
		diag = KDB_BADWIDTH;
		lkmd_printf("kdb_getphysword: bad width %ld\n", (long) size);
	}
	return(diag);
}
...
int kdb_getword(unsigned long *word, unsigned long addr, size_t size)
{
	...
	*word = 0;	/* Default value if addr or size is invalid */
	...
	default:
		diag = KDB_BADWIDTH;
		lkmd_printf("kdb_getword: bad width %ld\n", (long) size);
	}
	return(diag);
}
...
int kdb_putword(unsigned long addr, unsigned long word, size_t size)
{
	...
	default:
		diag = KDB_BADWIDTH;
		lkmd_printf("kdb_putword: bad width %ld\n", (long) size);
	}
...
kdb_task_state_string(const char *s)
{
	...
	while (*s) {
		...
		default:
			  lkmd_printf("%s: unknown flag '%c' ignored\n", __FUNCTION__, *s);
			  break;
		}
		++s;
	}
...
int kdb_save_running(struct pt_regs *regs, kdb_reason_t reason,
...
void kdb_unsave_running(struct pt_regs *regs)
{
	struct kdb_running_process *krp = kdb_running_process + smp_processor_id();
...
int kdb_getuserarea_size(void *to, unsigned long from, size_t size)
{
	...
	page = kdb_get_one_user_page(lkmd_current_task, from, size, 0);
	if (!page)
		return size;
...
int kdb_putuserarea_size(unsigned long to, void *from, size_t size)
{
	...
	page = kdb_get_one_user_page(lkmd_current_task, to, size, 1);
	if (!page)
		return size;
...
debug_kusage(void)
{
	...
	lkmd_printf("%s: debug_kmalloc memory leak dah_first %d\n",
		   __FUNCTION__, dah_first);
	if (dah_first) {
		h_used = (struct debug_alloc_header *)debug_alloc_pool;
		lkmd_printf("%s: h_used %p size %d\n", __FUNCTION__, h_used, h_used->size);
	}
	do {
		h_used = (struct debug_alloc_header *)
			  ((char *)h_free + dah_overhead + h_free->size);
		lkmd_printf("%s: h_used %p size %d caller %p\n",
