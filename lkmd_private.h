...
//#define KDB_DEBUG_STATE(text,value)	kdb_print_state(text, value)
...
//extern int kallsyms_symbol_complete(char *prefix_name, int max_len);
	 ...
	 */
extern int lkmd_register(char *, kdb_func_t, char *, char *, short);
extern int lkmd_register_repeat(char *, kdb_func_t, char *, char *, short, kdb_repeat_t);
extern int lkmd_unregister(char *);
...
extern int kdbgetaddrarg(int, const char**, int*, unsigned long *,
			 long *, char **);
extern int lkmd_get_sym_val(const char *, kdb_symtab_t *);
...
#define KDB_NULL_REGS(regs) \
	(regs == (struct pt_regs *)NULL ? lkmd_printf("%s: null regs - should never happen\n", __FUNCTION__), 1 : 0)
...
#define KDB_STATE_PRINTF_LOCK	0x00001000	/* Holds lkmd_printf lock */
...
extern void kdb_id_init(void);
	 ...
	 */
extern int lkmda_init(void);
extern void lkmda_exit(void);
extern int kdb_io_init(void);
...
// extern const char *lkmd_walk_kallsyms(loff_t *pos);
...
extern int kdb_task_has_cpu(const struct task_struct *p);
...
extern const struct task_struct *lkmd_current_task;
...
struct task_struct *lkmd_find_task_by_pid_ns(pid_t, struct pid_namespace *);
struct task_struct *lkmd_curr_task(int);
void lkmd_irq_enter(void);
void lkmd_irq_exit(void);
int lkmd_kernsym_init(void);

extern void kdb_kbd_cleanup_state(void);

extern int lkmd_strnicmp(const char *s1, const char *s2, size_t len);
