 ...
 * a non-maskable entry method. The vector is LKMD_VECTOR,
 * defined in hw_irq.h
 */
#define KDB_ENTER()	do {if (kdb_on && !KDB_IS_RUNNING()) { asm("\tint3\n"); }} while(0)
...
static inline int __kdba_putarea_size(unsigned long to_xxx, void *from, size_t size)
{
...
static inline int __kdba_getarea_size(void *to, unsigned long from_xxx, size_t size)
{
	mm_segment_t oldfs = get_fs();
	int r;
	*((volatile char *)to) = '\0';
...
static inline int kdba_verify_rw(unsigned long addr, size_t size)
{
...
static inline unsigned long kdba_funcptr_value(void *fp)
{
