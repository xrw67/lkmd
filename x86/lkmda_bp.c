...
kdb_dbtrap_t kdba_db_trap(struct pt_regs *regs, int error_unused)
{
	kdb_machreg_t dr6;
	kdb_machreg_t dr7;
	int rw, reg;
	...
	if (KDB_DEBUG(BP))
		lkmd_printf("kdb: dr6 0x%lx dr7 0x%lx\n", dr6, dr7);
	if (dr6 & DR6_BS) {
		if (KDB_STATE(SSBPT)) {
			if (KDB_DEBUG(BP))
				lkmd_printf("ssbpt\n");
			...
			for(i=0,bp=kdb_breakpoints;
			    ...
			    i++, bp++) {
				if (KDB_DEBUG(BP))
					lkmd_printf("bp 0x%p enabled %d delayed %d global %d cpu %d\n",
				...
				if (!bp->bp_global && bp->bp_cpu != smp_processor_id())
					continue;
				if (KDB_DEBUG(BP))
					lkmd_printf("bp for this cpu\n");
				if (bp->bp_delayed) {
					bp->bp_delayed = 0;
					if (KDB_DEBUG(BP)){
						/* Can't be hw breakpoint */
						if (bp->bp_hardtype)
							lkmd_printf("kdb: Error - hw bp delayed\n");
						lkmd_printf("kdba_installbp\n");
					}
			...
			if (i == KDB_MAXBPT) {
				lkmd_printf("kdb: Unable to find delayed breakpoint\n");
			}
			if (!KDB_STATE(DOING_SS)) {
				regs->flags &= ~X86_EFLAGS_TF;
				return(KDB_DB_NOBPT);
...
handle:
	...
	for(i = 0, bp = kdb_breakpoints; i < KDB_MAXBPT; i++, bp++) {
		if (!(bp->bp_free)
			...
			/* Hit this breakpoint. */
			lkmd_printf("%s breakpoint #%d at " kdb_bfd_vma_fmt "\n",
				  kdba_rwtypes[rw],
				  i, bp->bp_addr);
...
kdb_dbtrap_t kdba_bp_trap(struct pt_regs *regs, int error_unused)
{
	int i;
	kdb_dbtrap_t rv;
	kdb_bp_t *bp;
	...
	/* Determine which breakpoint was encountered. */
	if (KDB_DEBUG(BP))
		lkmd_printf("kdba_bp_trap: ip=0x%lx (not adjusted) flags=0x%lx regs=0x%p sp=0x%lx\n",
	...
	for(i = 0, bp = kdb_breakpoints; i < KDB_MAXBPT; i++, bp++) {
		...
		if (!bp->bp_global && bp->bp_cpu != smp_processor_id())
		 ...
		 if ((void *)bp->bp_addr == (void *)(regs->ip - bp->bp_adjust)) {
			...
			lkmd_printf("Instruction(i) breakpoint #%d at 0x%lx (adjusted)\n", i, regs->ip);
			kdb_id1(regs->ip);
			rv = KDB_DB_BPT;
			bp->bp_delay = 1;
...
static void kdba_handle_bp(struct pt_regs *regs, kdb_bp_t *bp)
{
	if (KDB_NULL_REGS(regs))
		return;

	if (KDB_DEBUG(BP))
		lkmd_printf("regs->ip = 0x%lx\n", regs->ip);
...
static void kdba_printbpreg(kdbhard_bp_t *bph)
{
	lkmd_printf(" in dr%ld", bph->bph_reg);
}
...
int kdba_parsebp(int argc, const char **argv, int *nextargp, kdb_bp_t *bp)
{
	...
	if ((argc + 1) != nextarg) {
		if (lkmd_strnicmp(argv[nextarg], "datar", sizeof("datar")) == 0) {
			bph->bph_mode = 3;
		} else if (lkmd_strnicmp(argv[nextarg], "dataw", sizeof("dataw")) == 0) {
			bph->bph_mode = 1;
		} else if (lkmd_strnicmp(argv[nextarg], "io", sizeof("io")) == 0) {
			bph->bph_mode = 2;
		} else if (lkmd_strnicmp(argv[nextarg], "inst", sizeof("inst")) == 0) {
			bph->bph_mode = 0;
		} else {
			return KDB_ARGCOUNT;
		}
		...
		if (KDB_DEBUG(BP))
			lkmd_printf("kdba_bp: no args, forcehw is %d\n", bp->bp_forcehw);
		if (bp->bp_forcehw) {
			/*
	...
	if (bph->bph_mode != 2 && kdba_verify_rw(bp->bp_addr, bph->bph_length+1)) {
		lkmd_printf("Invalid address for breakpoint, ignoring bp command\n");
		return KDB_BADADDR;
	}
...
static kdbhard_bp_t *kdba_allocbp(kdbhard_bp_t *bph, int *diagp, unsigned int cpu)
{
	int i;
	kdbhard_bp_t *newbph;
...
void kdba_alloc_hwbp(kdb_bp_t *bp, int *diagp)
{
	int i;
...
static void kdba_freebp(kdbhard_bp_t *bph)
{
	bph->bph_free = 1;
}
...
int kdba_installbp(struct pt_regs *regs, kdb_bp_t *bp)
{
	int cpu = smp_processor_id();
	...
	if (KDB_DEBUG(BP)) {
		lkmd_printf("kdba_installbp bp_installed %d\n", bp->bp_installed);
	}
	if (!KDB_STATE(SSBPT))
		bp->bp_delay = 0;
	...
	if (bp->bp_hardtype) {
		if (KDB_DEBUG(BP) && !bp->bp_global && cpu != bp->bp_cpu){
			lkmd_printf("kdba_installbp: cpu != bp->bp_cpu for local hw bp\n");
		}

		if (KDB_DEBUG(BP) && !bp->bp_hard[cpu]){
			lkmd_printf("kdba_installbp: Error - bp_hard[smp_processor_id()] is emply\n");
			return 1;
		}
		...
		if (!bp->bp_hard[cpu]->bph_installed){
			...
			if (KDB_DEBUG(BP)) {
				lkmd_printf("kdba_installbp hardware reg %ld at " kdb_bfd_vma_fmt "\n",
		...
		if (bp->bp_delay) {
			if (KDB_DEBUG(BP))
				lkmd_printf("kdba_installbp delayed bp\n");
			kdba_handle_bp(regs, bp);
		} else {
			if (kdb_getarea_size(&(bp->bp_inst), bp->bp_addr, 1) ||
			    kdb_putword(bp->bp_addr, IA32_BREAKPOINT_INSTRUCTION, 1)) {
				lkmd_printf("kdba_installbp failed to set software breakpoint at " kdb_bfd_vma_fmt "\n", bp->bp_addr);
				return(1);
			}
			bp->bp_installed = 1;
			if (KDB_DEBUG(BP))
				lkmd_printf("kdba_installbp instruction 0x%x at " kdb_bfd_vma_fmt "\n",
...
int kdba_removebp(kdb_bp_t *bp)
{
	 ...
	 */
	if (KDB_DEBUG(BP)) {
		lkmd_printf("kdba_removebp bp_installed %d\n", bp->bp_installed);
	}

	if (bp->bp_hardtype) {
		if (KDB_DEBUG(BP) && !bp->bp_global && cpu != bp->bp_cpu){
			lkmd_printf("kdba_removebp: cpu != bp->bp_cpu for local hw bp\n");
		}

		if (KDB_DEBUG(BP) && !bp->bp_hard[cpu]){
			lkmd_printf("kdba_removebp: Error - bp_hard[smp_processor_id()] is emply\n");
			return 1;
		}

		if (KDB_DEBUG(BP)) {
			lkmd_printf("kdb: removing hardware reg %ld at " kdb_bfd_vma_fmt "\n",
				   bp->bp_hard[cpu]->bph_reg, bp->bp_addr);
		}
		...
		if (KDB_DEBUG(BP))
			lkmd_printf("kdb: restoring instruction 0x%x at " kdb_bfd_vma_fmt "\n",
