...
/* ARGSUSED */
static int kdba_dis_getsym(bfd_vma addr, disassemble_info *dip)
{
...
/* ARGSUSED */
static void kdba_printaddress(kdb_machreg_t addr, disassemble_info *dip, int flag)
{
	kdb_symtab_t symtab;
	int spaces = 5;
	unsigned int offset;
	...
	if (symtab.sym_name) {
		 ...
		 * lkmd_printf but we want dip->fprintf_func.
		 */
...
static void kdba_dis_printaddr(bfd_vma addr, disassemble_info *dip)
{
...
/* ARGSUSED */
static int kdba_dis_getmem(bfd_vma addr, bfd_byte *buf, unsigned int length, disassemble_info *dip)
{
